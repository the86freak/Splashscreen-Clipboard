...
unsigned int __stdcall CSplashWnd::SplashThreadProc( void* lpParameter )
{
  ...
  else
  {
	...
	std::wstring a = L"Lade Daten...";
	pThis->SetProgress(3, (a.c_str()));
  ...
  switch (uMsg)
	{
		case WM_PAINT:
		{
			if (pInstance->m_pImage)
			{
				...
				{
					Gdiplus::Font msgFont( L"Tahoma", 12, Gdiplus::UnitPixel );
					Gdiplus::SolidBrush msgBrush( static_cast<DWORD>(Gdiplus::Color::Black) );
					gdip.DrawString( pInstance->m_ProgressMsg.c_str(), -1, &msgFont, Gdiplus::PointF(2.0f, pInstance->m_pImage->GetHeight()-45.0f), &msgBrush );
				}
			}
